...
bool run_downscaling_decoding_tests(std::filesystem::path input_base_dir) {
    ...
    for (const auto& file_path : input_files_paths) {
        ...
        if (decoder.decode(decoded_img, 0, 0, 100, 100, writer)) {
            ...
            if (!write_pgm(output_file_path, decoded_img, dst_dims)) {
                ...
                test_passed = false;
                std::cout << "\n  Writing output at " << DST_WIDTH_PX << "x" << DST_HEIGHT_PX << " px FAILED.\n";
            }
        }
        else {

            test_passed = false;
            std::cout << "\n  Decoding 800x800 px JPEG w/ rescaling to " << DST_WIDTH_PX << "x" << DST_HEIGHT_PX << " px FAILED.\n";
        }
