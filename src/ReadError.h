...
/// reports a single bit read failure to its caller on running out of buffer from
/// which to read. The caller may have needed the requested bit to construct its
/// own return value. It now needs to communicate its own failure using a different
/// error code - one that fits into its return value type and sits outside
/// of its valid return value domain. The caller can also fail due to being
/// "successfully" served a bit from an otherwise corrupted ECS. Therefore
